...
\centering
    \begin{columns}
        ...
        \end{itemize}
         ...
         If we see integrating an $n-1$-form on $\mathcal{N}$, as counting the planes entering $\mathcal{N}$, we conclude that solving the problem of integrating $\omega$ over $\mathcal{N}$ is the same as to count the number of line ends in $\mathcal{M}$. That is,
            ...
            \begin{minipage}[c]{0.1\textwidth}
                \includegraphics[scale=0.2]{stokesthm.png}
            \end{minipage}
            \qquad
            \begin{minipage}[c]{0.25\textwidth}
                \includegraphics[scale=0.8]{stokes.png}
            \end{minipage}
        ...
        \block{References}{
                \begin{biblist}
                    \bib{1}{misc}{
                      title={On the Visualization of Differential Forms (Finished except for appendix)},
                      author={ist189623},
                      year={2021},
                      month={September},
